...
class BackendManagerControllerTest extends UnitTestCase
{
    ...
    {
        $this->mockedBackendManagerController = $this->getAccessibleMock(BackendManagerController::class, ['dummy'], [], '', false);
    }
    ...
    {
        [$category1, $category2] = $this->getCategories();

        $mockedProductRepository = $this->createPartialMock(ProductRepository::class, ['findAllProductsByCategories']);
        $mockedProductRepository->expects($this->exactly(2))->method('findAllProductsByCategories');
        $this->mockedBackendManagerController->_set('productRepository', $mockedProductRepository);

        $expect = [12, 21];
        $result = array_keys($this->mockedBackendManagerController->_call('getCategoriesWithProducts', [$category1, $category2]));
    ...
    {
        $this->assertCount(0, $this->mockedBackendManagerController->_call('getCategoriesWithProducts', []));
    }
    ...
    {
        [$category1, $category2, $category3] = $this->getCategories();

        $result = [$category1, $category2, $category3];

        $this->assertEquals($result, $this->mockedBackendManagerController->_call('buildCategoryBreadCrumbs', $category3));
    }
    ...
    public function generateCorrectPositioningArrayFromRecordsArray()
    {
        ...
        $this->assertEquals($expect, $this->mockedBackendManagerController->_call('generatePositionsArray', [
            $category1,
            $category2,
            $category3
        ]));
