...
class ItemsProcessingFunction
{
    ...
    public function getListOfProductsWithinCategories(array &$data, TcaSelectItems $pObj)
    {
        if (!empty($data['row']['settings.customProductsList.productsCategories'])) {
            ...
            $categories = GeneralUtility::intExplode(',', $data['row']['settings.customProductsList.productsCategories'], true);
            foreach ($categories as $category) {
                /** @var CategoryCollection $collection */
                $collection = CategoryCollection::load($category, true, 'tx_pxaproductmanager_domain_model_product', 'categories');
