...
class SolrIndexSingleAttributeValue
{
     ...
     * @return int|string
     */
    public function getSingleAttributeValue(/** @noinspection PhpUnusedParameterInspection */ // @codingStandardsIgnoreStart
        string $content, array $parameters// @codingStandardsIgnoreEnd
    )
    {
        ...
        if (is_array($attribute) && (isset($attributeValues[$attribute['uid']]) || $this->isFalType($attribute['type']))) {
            switch ($attribute['type']) {
                ...
                case Attribute::ATTRIBUTE_TYPE_FILE:
                    ...
                    $fileCollector->addFilesFromRelation('tx_pxaproductmanager_domain_model_product', TCAUtility::ATTRIBUTE_FAL_FIELD_NAME, $this->cObj->data);
    ...
    protected function getAttribute(string $identifier)
    {
        ...
        $row = $queryBuilder->select('uid', 'type')->from('tx_pxaproductmanager_domain_model_attribute')->where($queryBuilder->expr()->eq('sys_language_uid', $queryBuilder->createNamedParameter(0, Connection::PARAM_INT)), $queryBuilder->expr()->eq('identifier', $queryBuilder->createNamedParameter($identifier, Connection::PARAM_STR)))->setMaxResults(1)->execute()->fetch();
