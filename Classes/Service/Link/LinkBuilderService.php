...
class LinkBuilderService
{
     ...
     * @return string
     */
    public function buildForProduct(int $pageUid, $product, $category = null, bool $excludeCategories = false, bool $absolute = false): string
    {
        $arguments = [];
    ...
    protected function getCategoriesArguments(int $categoryUid): array
    {
        ...
        // Get parent
        $parentUid = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('sys_category')->select(['parent'], 'sys_category', ['uid' => $categoryUid])->fetchColumn(0);
        ...
        while ($parentUid) {
            ...
            $parentRow = $queryBuilder->select('uid', 'parent')->from('sys_category')->where($expr->eq('uid', $queryBuilder->createNamedParameter($parentUid, \PDO::PARAM_INT)), $expr->neq('parent', $queryBuilder->createNamedParameter(0, \PDO::PARAM_INT)), $expr->notIn('uid', $queryBuilder->createNamedParameter($categories, Connection::PARAM_INT_ARRAY)))->execute()->fetch();
            // Save result
    ...
    protected function buildUri(int $pageUid, string $action, array $arguments, bool $absolute = false): string
    {
        ...
        $parameters = GeneralUtility::implodeArrayForUrl(static::NAMESPACES, $arguments);
        ...
        $contentObjectRenderer = GeneralUtility::makeInstance(ContentObjectRenderer::class, $this->typoScriptFrontendController);
     ...
     * @return string
     */
    public function buildForCategory(int $pageUid, $category, bool $absolute = false): string
    {
